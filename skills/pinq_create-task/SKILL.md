...
- `labels` (list of strings) â€” label names to apply.
...
6. **Fetch teams** â€” call `list_teams`; if only one team exists use it, otherwise pick by name match or ask.
7. **Confirm before creating** â€” show the user the draft (title, assignee, priority) in one compact block and ask for confirmation. Do not call `save_issue` yet.
8. **Create** â€” on confirmation, call `save_issue` with title, description, team, assignee, priority, and labels.
...
- **Gate â€” `list_issue_labels`:** only when labels were provided and need validation
