---
name: linear-task
description: Use when creating or filling in a Linear task — drafting titles, descriptions, acceptance criteria, labels, priority, and suggesting assignees. Triggers on: "create a Linear task", "fill in this task", "draft a task for", "what should this task include", "who should I assign this to", "help me write this ticket", "task description for", "Linear ticket", "open a task", "linear'da task aç", "task doldur", "task yaz", "kime atasam".
---
...
## Non-Goals
- Does not create or submit tasks in Linear directly — the user submits the final task.
...
- **Assume when:** priority or label is unspecified — default to Medium priority and the most relevant project label; state the assumption explicitly.
...
- Required sections (in order):
  ...
  3. **Acceptance Criteria** — bulleted list of independently verifiable done conditions
  4. **Suggested Labels** — comma-separated (derive from project and task type)
  5. **Suggested Priority** — Urgent / High / Medium / Low with one-line rationale
  6. **Suggested Assignee** — name + one-line rationale grounded in context docs; if team doc is absent, write "— (team doc not loaded)"
- Error format:
...
4. **Draft** — write title (≤80 chars), description (2–5 sentences), acceptance criteria (≥3 bullets).
5. **Suggest metadata** — derive labels, priority, and assignee from context docs.
6. **Verify** — all 6 sections present; no invented team members; no context doc content leaked wholesale; title ≤80 chars; acceptance criteria independently verifiable.
...
**Suggested Priority:** High — Rindle is the flagship product; notification support is a core retention driver ahead of store launch.

**Suggested Assignee:** — (team doc not loaded — add `.pinq-doq/context/team/members.md` to enable assignee suggestions)
...
**Suggested Labels:** `feature`, `internal` (assumed — context doc not loaded)

**Suggested Priority:** Medium (assumed — no project priority signal available)
...
## Tests
- **T1 Normal:** clear intent + known project → all 6 sections present, project matched from context docs, no invented data
